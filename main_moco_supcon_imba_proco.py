...
import time
import warnings
from utils import SupConLoss,calculate_tensor_center
import torch
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        coreset_sampler = torch.utils.data.distributed.DistributedSampler(coreset_dataset)
    else:
        train_sampler = None
        coreset_sampler=None

    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
        num_workers=args.workers, pin_memory=True, sampler=train_sampler, drop_last=True)
    coreset_loader = torch.utils.data.DataLoader(
        coreset_dataset, batch_size=args.batch_size, shuffle=(coreset_sampler is None),
        num_workers=args.workers, pin_memory=True, sampler=coreset_sampler, drop_last=True)
    ...
    if args.targeted:
        ...
        for epoch in range(pretrain_epochs):
            ...
            # train for one epoch
            train(train_loader,coreset_loader, model, optimizer, epoch, args, train_logger)
        ...
        for epoch in range(tsc_epochs):
            ...
            # train for one epoch
            train(train_loader,coreset_loader, model, optimizer, epoch + pretrain_epochs, args, train_logger)
...
def train(train_loader,coreset_loader, model, optimizer, epoch, args, logger):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        new=center_tensor_v1[targets]
        features_f1_1=torch.cat([f1.unsqueeze(1), center_tensor_v1[targets].unsqueeze(1)], dim=1)
        features_f1_2=torch.cat([f1.unsqueeze(1), center_tensor_v2[targets].unsqueeze(1)], dim=1)
        features_f2_1 = torch.cat([f2.unsqueeze(1), center_tensor_v1[targets].unsqueeze(1)], dim=1)
        features_f2_2 = torch.cat([f2.unsqueeze(1), center_tensor_v2[targets].unsqueeze(1)], dim=1)
        #calc symmetric prototype loss
        class_temperature=None
        loss_v1_1=criterition_center(features_f1_1,class_temperature,False)
        loss_v2_2=criterition_center(features_f2_2,class_temperature,False)
        loss_v1_2=criterition_center(features_f1_2,class_temperature,False)
        loss_v2_1=criterition_center(features_f2_1,class_temperature,False)
        loss_center= 0.06*(loss_v1_1 + loss_v2_2 + loss_v1_2 + loss_v2_1 )
        final_loss=(loss+ loss_center)/2
